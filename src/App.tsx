...
function Zoning({ d }: { d: any }) {
  ...
  return <><Warning>Capacity is a planning screen. {PUBLIC_DEMO ? 'Coordinates and zoning are fictional fixture context. ' : ''}A match does not establish legal development permission.</Warning><div className="metrics">{[['Statutory GFA', c.statutory_gfa_sqm], ['Physical GFA', c.physical_gfa_sqm], ['De facto GFA', c.de_facto_gfa_sqm], ['Economic GFA', c.economic_gfa_sqm]].map(([label, value]) => <Metric key={label} l={label} v={number(value) + ' sqm'}/>)}</div>
