...
class TwoStageRequest(BaseModel):capital_budget_m:float=500;minimum_liquidity_m:float=50;max_concurrent_projects:int=8;minimum_noi_ratio:float=.7;cvar_penalty:float=.25
...
init_db()
@app.get('/health')
def health():return {'status':'ok','date':'2026-09-24','model_version':'economic-model-v3-digital-twins-0.3','portfolio_loaded':PORTFOLIO.exists() or (EXAMPLES/'demo_portfolio.json').exists(),'demo_mode':DEMO_MODE or not PORTFOLIO.exists(),'ura_index_loaded':URA_DB.exists(),'results_loaded':RESULTS.exists()}
@app.get('/data/status')
def data_status():
