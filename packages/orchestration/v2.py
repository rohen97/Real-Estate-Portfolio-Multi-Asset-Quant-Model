...
def run_twin(twin:dict,model_dir:Path,legacy_input:dict|None=None,seed=20260923):
 ...
 action_values=evaluate_actions(current_value,current_noi,capacity.get('residual_value_m',0));risk=simulate_actions(action_values,draws=5000,seed=seed,asset_id=asset['asset_id'],common_factor_seed=20260923);options=real_options(risk,current_noi);preferred=max(risk,key=lambda x:x['expected_npv_m']-.3*max(0,x['cvar_95_m']));zone=(asset.get('ura_zoning',{}).get('matches') or [{}])[0];surroundings=surrounding_development_context(ROOT/'data/processed/ura_mp2025.sqlite',asset['latitude'],asset['longitude'],500,zone.get('objectid')) if asset.get('latitude') is not None else {'status':'unavailable'};market=context_for_asset(asset,load_snapshot(ROOT));selection=evaluate_selection({**asset,'synthetic_financials':bool(twin.get('synthetic_history',True))},capacity,risk,surroundings,market,current_value,current_noi,.078,5,load_transformation_zones(ROOT));preferred=next((x for x in risk if x['action']==selection['preferred_action']),preferred)
