...
from packages.orchestration.provenance import model_provenance,result_key,MODEL_VERSION
...
def compatible_zone(asset,match):
 ...
 if any(x in segments for x in ['industrial','factory','warehouse','logistics']):allowed+=['BUSINESS','INDUSTRIAL','WHITE']
 if 'mall' in segments or 'retail' in segments:allowed+=['COMMERCIAL','COMMERCIAL & RESIDENTIAL','WHITE']
 if 'hotel' in segments or 'hospitality' in segments or 'serviced residence' in segments:allowed+=['HOTEL','COMMERCIAL','WHITE','RESIDENTIAL']
...
def inputs_for_asset(asset):
 ...
 return ZoningAssumptions(site_area_sqm=area,current_gfa_sqm=current_gfa,legal_gpr=gpr,site_coverage_max=coverage,height_max_m=height,approval_probability=.62),proxy_economics(asset),notes, 'provisional_proxy_run',('low - fictional public demonstration inputs' if asset.get('synthetic') else 'low - portfolio workbook is a catalogue, not an underwriting dataset')
def run_asset(asset,seed=20260922,common_factor_seed=None,provenance=None):
 if asset.get('country')!='Singapore':return {**asset,'model_status':'outside_singapore_scope','zoning':None,'verification_required':True}
 assumptions,economics,notes,status,quality=inputs_for_asset(asset);capacity=calculate_capacity(assumptions,seed);approval=approval_path(seed=seed);base=dcf(economics);actions=simulate_actions(value_actions(economics,capacity.__dict__),seed=seed,asset_id=asset['asset_id'],common_factor_seed=common_factor_seed);preferred=max(actions,key=lambda x:x['expected_npv_m']-.3*max(0,x['cvar_95_m']));required=economics.discount_rate;expected_return=max(-.5,min(.5,(preferred['expected_npv_m']/max(economics.current_value_m,1))/max(preferred['execution_years'],.5)));verification=status!='observed_run' or bool(notes);market_context=context_for_asset(asset,load_snapshot(ROOT));zone=zone_match(asset);surroundings=surrounding_development_context(ROOT/'data/processed/ura_mp2025.sqlite',asset['latitude'],asset['longitude'],500,zone.get('objectid') if zone else None) if asset.get('latitude') is not None else {'status':'unavailable'};selection=evaluate_selection(asset,capacity.__dict__,actions,surroundings,market_context,economics.current_value_m,economics.current_noi_m,required,5,load_transformation_zones(ROOT))
 provenance=provenance or model_provenance(ROOT)
 selected_action=selection['preferred_action'];selected_case=next((x for x in actions if x['action']==selected_action),preferred)
 return {**asset,'market_context':market_context,'surrounding_context':surroundings,'selection':selection,'model_status':status,'input_quality':quality,'proxy_inputs':notes,'zoning_source':asset.get('ura_zoning'),'zoning_model_status':'verified_match' if compatible_zone(asset,zone_match(asset)) and not asset.get('ura_zoning',{}).get('spatial_review_required',False) else 'review_required','statutory_gpr_used':assumptions.legal_gpr,'gpr_source':'fictional_zoning_fixture' if asset.get('synthetic') else 'MP2025' if compatible_zone(asset,zone_match(asset)) else 'asset_class_proxy','capacity':capacity.__dict__,'approval':approval,'economics':economics.__dict__,'dcf':base,'actions':actions,'recommendation':{'action':selected_action,'management_label':{'Hold':'Retain','Retrofit':'Retrofit','Repurpose':'Repurpose','Redevelop':'Repurpose','Sell':'Release'}[selected_action],'expected_npv_m':selected_case['expected_npv_m'],'probability_of_loss':selected_case['probability_of_loss'],'selection_signal':selection['signal'],'explanation':f"{selected_action} maximises expected incremental NPV less the nonnegative CVaR penalty, before portfolio constraints. Selection screen: {selection['signal']}. {'Observed company inputs are in use.' if status=='observed_run' else 'Replace proxy inputs before decision use.'}"},'expected_alpha':selection['annual_expected_alpha'],'required_return':required,'model_version':MODEL_VERSION,'provenance':provenance,'verification_required':verification or not selection['readiness']['ready'],'cache_key':result_key(asset,seed,provenance)}
def run_portfolio(assets,seed=20260922):
 provenance=model_provenance(ROOT)
 return [run_asset(asset,seed+int(sha256(asset['asset_id'].encode()).hexdigest()[:8],16)%1000000,common_factor_seed=seed,provenance=provenance) for asset in assets]
