...
def evaluate_actions(current_value_m,base_noi_m,capacity_option_m,discount_rate=.078,horizon=10):
 results=[]
 base=[base_noi_m*(1.02)**y for y in range(1,horizon+1)];base[-1]+=base_noi_m*(1.02)**horizon/.05;base_pv=npv(discount_rate,base)
 for spec in ACTIONS:
  if spec.action=='Sell':after=current_value_m*.975;capex=-after;failure=0;cash=[after]
   ...
   cash[-1]+=base_noi_m*(1+spec.noi_uplift)*(1.025)**horizon/.05*(1+spec.terminal_uplift)+capacity_option_m*(.2 if spec.action=='Repurpose' else .55 if spec.action=='Redevelop' else 0);after=npv(discount_rate,cash);failure=capex*(1-spec.success_probability)*.55
